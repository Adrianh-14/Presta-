...
[ApiController, Route("api/tenant/config"), Authorize]
public sealed class TenantConfigurationController : ControllerBase
{
    ...
    [HttpPut, Authorize(Policy = AuthorizationPolicies.ManageUsers)] public async Task<IActionResult> Put(UpdateTenantBrandingRequest request, CancellationToken ct) => await _service.UpdateAsync(TenantId(),request,ct) is { } result ? Ok(result) : NotFound();

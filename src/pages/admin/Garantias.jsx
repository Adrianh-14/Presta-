...
export default function Garantias() {
  ...
  const openContract = async (path) => {
    try {
      const { data } = await api.get(`/api/media/${path}`, { responseType: 'blob' });
      const url = URL.createObjectURL(data);
      window.open(url, '_blank', 'noopener,noreferrer');
      setTimeout(() => URL.revokeObjectURL(url), 60000);
    } catch (error) {
      window.alert(error.response?.data?.message || 'No se pudo abrir el contrato.');
    }
  };
  useEffect(() => {

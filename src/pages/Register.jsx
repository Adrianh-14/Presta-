...
import { ArrowLeft, ArrowRight, BadgeCheck, Building2, Check, ChevronLeft, Eye, EyeOff, ImagePlus, LocateFixed, LockKeyhole, MailCheck, ShieldCheck, UserRound, Upload, X } from 'lucide-react';
...
export default function Register() {
  ...
  const update = (field) => (event) => { const value = event.target.type === 'checkbox' ? event.target.checked : event.target.value; setForm((current) => ({ ...current, [field]: value })); setFieldErrors((current) => { const next = { ...current }; delete next[field]; return next; }); setError(''); if (field === 'email') { setEmailVerified(false); setEmailCodeSent(false); setEmailCode(''); setEmailVerificationOpen(false); setEmailVerificationError(''); } };
  ...
  const validateStep = () => {
    const required = {
      ...
      1: [['country', 'Selecciona el país.'], ['address', 'Indica la dirección de la empresa.'], ['city', 'Indica la ciudad.'], ['province', 'Indica la provincia.'], ['employeeCount', 'Indica la cantidad de empleados.']],
  ...
  return (
    <main className="min-h-screen bg-slate-50 lg:grid lg:grid-cols-[0.78fr_1.22fr]">
      <aside className="financial-grid hidden min-h-screen p-10 text-white lg:flex lg:flex-col lg:justify-between xl:p-16">
        ...
        <p className="text-xs leading-5 text-slate-400">Tus documentos se almacenan en un espacio privado de tu empresa. El sistema es gratis y no requiere tarjeta.</p>
      </aside>
      ...
      <section className="flex min-h-screen items-start justify-center px-5 py-8 sm:px-10 xl:px-20">
        <div className="w-full max-w-2xl">
          ...
          <form onSubmit={submit} className="mt-7 rounded-16 border border-surface-border bg-white p-5 shadow-card sm:p-7">
            ...
            {step === 3 && <Step title="Acceso administrador" icon={LockKeyhole} description="Estos datos te permitirán entrar al panel de tu empresa."><Field label="Correo de trabajo" required error={fieldErrors.email}><input autoFocus type="email" autoComplete="email" className="auth-input" value={form.email} onChange={update('email')} onBlur={handleEmailBlur} placeholder="nombre@empresa.com" />{emailVerified ? <span className="mt-1 block text-xs font-semibold text-success-600">✓ Correo verificado</span> : emailCodeSent ? <span className="mt-1 block text-xs font-semibold text-accent-700">Código enviado · verifica tu correo para continuar</span> : <span className="mt-1 block text-xs font-normal text-slate-400">Al terminar de escribirlo te pediremos verificarlo.</span>}</Field><Field label="Contraseña segura" required error={fieldErrors.password}><div className="relative"><input type={showPassword ? 'text' : 'password'} autoComplete="new-password" className="auth-input px-10" value={form.password} onChange={update('password')} minLength={12} placeholder="12+ caracteres" /><button type="button" onClick={() => setShowPassword((value) => !value)} aria-label={showPassword ? 'Ocultar contraseña' : 'Mostrar contraseña'} className="absolute right-3.5 top-1/2 -translate-y-1/2 text-slate-400">{showPassword ? <EyeOff size={18} /> : <Eye size={18} />}</button></div></Field><div className="rounded-12 border border-surface-border bg-surface-fill p-4"><p className="text-xs font-bold text-navy-700">Antes de crear tu espacio</p><div className="mt-3 grid gap-2 text-xs text-slate-500 sm:grid-cols-2">{['Capital inicial bajo tu control', 'Perfil legal documentado', 'Espacio aislado por empresa', 'Acceso gratis sin tarjeta'].map((item) => <span key={item} className="flex items-center gap-2"><Check size={13} className="text-success-600" />{item}</span>)}</div></div><label className={`flex cursor-pointer items-start gap-3 text-xs leading-5 ${fieldErrors.acceptTerms ? 'text-danger-600' : 'text-slate-500'}`}><input type="checkbox" checked={form.acceptTerms} onChange={update('acceptTerms')} className="mt-1 h-4 w-4 rounded border-surface-border text-accent-600" /><span>Acepto los términos de servicio y la política de privacidad de PréstamoPlus.</span></label></Step>}
            <div className="mt-7 flex items-center justify-between gap-3 border-t border-surface-border pt-5"><button type="button" onClick={previous} disabled={step === 0 || loading} className="inline-flex items-center gap-1.5 rounded-8 px-3 py-2.5 text-sm font-bold text-slate-500 hover:bg-slate-50 disabled:invisible"><ChevronLeft size={17} /> Atrás</button>{step < steps.length - 1 ? <button type="button" onClick={next} disabled={emailVerificationLoading} className="inline-flex items-center gap-2 rounded-8 bg-navy-800 px-5 py-3 text-sm font-bold text-white transition hover:bg-navy-700 disabled:opacity-60">{emailVerificationLoading ? 'Verificando…' : step === 3 && !emailVerified ? (emailCodeSent ? 'Verificar correo' : 'Enviar código') : 'Continuar'} <ArrowRight size={17} /></button> : <button type="submit" disabled={loading || !emailVerified} className="inline-flex items-center gap-2 rounded-8 bg-accent-600 px-5 py-3 text-sm font-bold text-white transition hover:bg-accent-700 disabled:cursor-wait disabled:opacity-60">{loading ? 'Creando espacio…' : <>Crear mi cuenta <ArrowRight size={17} /></>}</button>}</div>
          </form>
      ...
      {emailVerificationOpen && <div className="fixed inset-0 z-50 flex items-center justify-center bg-navy-950/45 p-4" role="presentation"><section role="dialog" aria-modal="true" aria-labelledby="email-verification-title" className="w-full max-w-md rounded-16 border border-surface-border bg-white p-6 shadow-card-lg"><div className="flex items-start justify-between gap-4"><div className="flex items-start gap-3"><span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-12 bg-accent-50 text-accent-600"><MailCheck size={21} /></span><div><h2 id="email-verification-title" className="font-display text-xl font-extrabold text-navy-800">Verifica tu correo</h2><p className="mt-1 text-sm leading-5 text-slate-500">Necesitamos confirmar que puedes recibir mensajes en esta dirección antes de crear tu empresa.</p></div></div><button type="button" onClick={() => setEmailVerificationOpen(false)} className="rounded-full p-2 text-slate-400 hover:bg-slate-100" aria-label="Cerrar verificación"><X size={18} /></button></div><div className="mt-5 rounded-12 border border-accent-100 bg-accent-50/40 p-4"><p className="text-xs font-semibold uppercase tracking-wide text-accent-700">Correo a verificar</p><p className="mt-1 break-all font-semibold text-navy-800">{form.email}</p></div>{emailCodeSent ? <div className="mt-5"><label className="text-sm font-semibold text-navy-700">Código de 6 dígitos<input autoFocus inputMode="numeric" maxLength={6} className="auth-input mt-2 text-center font-mono text-xl tracking-[0.35em]" value={emailCode} onChange={(event) => { setEmailCode(event.target.value.replace(/\D/g, '')); setEmailVerificationError(''); }} placeholder="000000" /></label><p className="mt-2 text-xs text-slate-500">Revisa tu bandeja de entrada. El código vence en 15 minutos.</p></div> : <p className="mt-5 text-sm text-slate-600">Pulsa el botón para enviarte el código de verificación.</p>}{emailVerificationError && <p role="alert" className="mt-3 rounded-8 bg-danger-50 px-3 py-2 text-sm font-semibold text-danger-600">{emailVerificationError}</p>}<div className="mt-6 flex justify-end gap-3"><button type="button" onClick={() => setEmailVerificationOpen(false)} className="rounded-8 px-4 py-2.5 text-sm font-semibold text-slate-500 hover:bg-slate-100">Después</button><button type="button" disabled={emailVerificationLoading} onClick={async () => { setEmailVerificationLoading(true); setEmailVerificationError(''); try { if (!emailCodeSent) { await authService.requestEmailVerification(form.email.trim(), 'tenant-registration'); setEmailCodeSent(true); } else { await authService.confirmEmailVerification(form.email.trim(), 'tenant-registration', emailCode.trim()); setEmailVerified(true); setEmailVerificationOpen(false); setError('Correo verificado correctamente. Ya puedes crear tu empresa.'); } } catch (requestError) { setEmailVerificationError(requestError.response?.data?.message || 'No pudimos verificar el correo.'); } finally { setEmailVerificationLoading(false); } }} className="rounded-8 bg-accent-600 px-4 py-2.5 text-sm font-bold text-white hover:bg-accent-700 disabled:cursor-wait disabled:opacity-60">{emailVerificationLoading ? 'Procesando…' : emailCodeSent ? 'Confirmar correo' : 'Enviar código'}</button></div></section></div>}
    </main>
  );
}
